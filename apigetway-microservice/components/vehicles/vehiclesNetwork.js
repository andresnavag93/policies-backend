...
async function get(req, res, next) {
  try {
    const endpoint = `vehicles/${req.params.id}`;
    const axiosResponse = await getData(axiosClientMicroservice, endpoint, {});
    response.success(req, res, axiosResponse.data.data, 200);
  } catch (error) {
    try {
...
async function create(req, res, next) {
  try {
    const endpoint = `vehicles/clients/${req.params.client_id}`;
...
async function update(req, res, next) {
  try {
    const endpoint = `vehicles/${req.params.id}/clients/${req.params.client_id}`;
